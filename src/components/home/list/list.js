  ...
  return (
          ...
          {activeTab === "Profiller" && 
            ...
            <div key={item.id} className="relative  text-white  rounded-xl ">
              ...
              <div className="text-white px-2 text-sm flex flex-row flex-nowrap items-center gap-2 font-bold   ">{item.fullName} {item.age}<Image  src={'/verified.png'} className="w-4 h-4" width={16} height={10} />  </div>
              <div className="text-white px-2 flex flex-row flex-nowrap items-center gap-2  text-sm    ">{item.country_data ? item.country_data.country_name == '🇹🇷' ? <Image src="/turkey.png" width={20} height={20} /> :  item.country_data.country_name:''}</div>
              </div>
            </div>
            </Link>
          }
